...
## 1.1预备知识: 
 ...
 (1)利普希茨区域的定义:如果 $$\Omega$$ 的边界能被利普希茨连续函数局部代替,则称 
    $$\Omega$$ 为一个利普希茨区域.即对任意的 $$x\in\partial\Omega$$ ,存在 $$x$$
    的一个邻域,使得 $$G\cap\partial\Omega$$ 在一个适当的坐标系下是一个利普希茨连续函数.
...
2.(1)**一些利普希茨区域的例子:**
 ...
 所有光滑的区域是利普希茨区域;
 非光滑区域的例子:在 $$R^2$$ 内的每一个多边形和 $$R^3$$ 内的每一个多面体都是利普
 希茨区域;在 $$R^n$$ 内每一个凸区域都是利普希茨区域(凸区域:区域上任意两点的连线都在区域内,则这样的区域就是凸区域).
